...
编辑生成的 `src/content/fpga-learning-note.md`，填写标签、摘要和正文，并在完成后将 `draft` 改为 `"false"`。先进行安全检查：
...
`updated`、`cover` 和 `series` 是可选字段；`draft: "true"` 的文章不会进入页面、搜索、RSS 或 Sitemap。一键发布会要求先将草稿状态改为 `"false"`。
...
确认文件位于 `src/content`，扩展名为 `.md`，Frontmatter 前后都使用三个连字符 `---`，并执行 `npm run validate:content` 查看具体错误。
