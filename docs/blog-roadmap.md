...
- 发布一篇文章仍需手动执行多条 Git 命令。
- Frontmatter 没有严格校验，也没有草稿机制。
- 没有自动测试和 CI 发布门禁。
- 所有 Markdown 正文和代码高亮都进入主包，当前主 JavaScript 约 695 kB。
- 生产统计依赖 Cloudflare 后台设置，源码中的统计只保存在单台设备的 localStorage。
- 站点描述仍偏向早期“前端博客”定位，与现在的 IC、FPGA、学习和生活内容不完全一致。
- 缺少文章封面、社交分享图和图片优化流程。
